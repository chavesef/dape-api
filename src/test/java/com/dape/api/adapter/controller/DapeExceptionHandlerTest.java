...
class DapeExceptionHandlerTest {
    ...
    void handleInvalidRequestParameters() {
        ...
        ResponseEntity<List<BetPostErrorResponse>> actualResponse = exceptionHandler.handleValidationException(exception);

        List<BetPostErrorResponse> expectedErrors = new ArrayList<>();
        expectedErrors.add(new BetPostErrorResponse(HttpStatus.BAD_REQUEST.value(), "Valor da odd deve ser maior que 1"));
        expectedErrors.add(new BetPostErrorResponse(HttpStatus.BAD_REQUEST.value(), "Descrição da aposta não deve ser nula/vazia"));
        ResponseEntity<List<BetPostErrorResponse>> expectedResponse =
